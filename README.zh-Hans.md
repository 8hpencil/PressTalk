...
- **Google 可能将免费层请求数据用于模型训练。** 这是 Google 对 AI Studio 免费层的政策，PressTalk 无法改变。敏感内容请使用付费层密钥，或等待路线图中的完全离线本地模型。
...
- **v1.1 —— 本地 whisper.cpp 模型**：完全离线转写，无需 API 密钥，数据不出本机；同时解决无法直连 Google API 地区的可用性。
- 基于同一 `TranscriptionProvider` 协议接入更多服务（OpenAI 兼容端点、国内 ASR 等）。
- **透明声明**：未来可能推出付费"省心版"——公证安装包、自动更新、托管转写额度（免申请密钥）、行业词库模板。无论如何，你眼前这份 GPL 核心代码保持开源且功能自足。
